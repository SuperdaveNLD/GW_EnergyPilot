...
def _recommended_emhass_options() -> dict[str, Any]:
    """Return form-shaped recommended EnergyPilot EMHASS settings."""
    return {
    ...
    *,
    ...
    return {
        ...
        "managed_values": [
            {
                "key": change_key,
                "current": config.get(change_key),
                "required": synced.get(change_key),
                "synchronized": config.get(change_key) == synced.get(change_key),
            }
            for change_key in (
...
async def websocket_get_emhass_sync(
    ...
    """Return recommended defaults and current EMHASS sync status."""
...
async def websocket_apply_emhass_sync(
    ...
    """Safely patch and verify the required EMHASS configuration values."""
...
def async_register_emhass_sync_api(hass: HomeAssistant) -> None:
    """Register EnergyPilot EMHASS sync commands once."""
