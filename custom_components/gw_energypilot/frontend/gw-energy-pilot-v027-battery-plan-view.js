...
function dimensions(size, modal) {
  if (modal || size === "large") {
    return { width: 1040, height: 470, left: 66, right: 118, top: 42, bottom: 52, xTicks: [0, 6, 12, 18, 24] };
  }
  if (size === "compact") {
    return { width: 760, height: 215, left: 45, right: 96, top: 26, bottom: 38, xTicks: [0, 12, 24] };
  }
  return { width: 920, height: 340, left: 58, right: 112, top: 36, bottom: 46, xTicks: [0, 6, 12, 18, 24] };
}
...
function chartSvg(panel, data, size, modal) {
  ...
  const { width, height, left, right, top, bottom, xTicks } = d;
  ...
  return `<svg viewBox="0 0 ${width} ${height}" width="100%" role="img" aria-label="${panel._escape(t(panel, "subtitle"))}"><defs><filter id="epV027PriceGlow" x="-20%" y="-40%" width="140%" height="180%"><feGaussianBlur stdDeviation="2.1" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter><pattern id="epV051UnknownHatch" width="5" height="5" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="5" height="5" fill="#697b8c" fill-opacity=".28"/><line x1="0" y1="0" x2="0" y2="5" stroke="#b7c3ce" stroke-width="1.2"/></pattern></defs>${titles}${grid}${powerTicks}${actualBars}${historicalPlan}${futurePlan}${actualSoc}${forecastSoc}${actualSocDots}${forecastSocDots}${socTicks}${priceLine}${priceDots}${priceTicks}${nowLine}</svg>`;
}
...
function legendHtml(panel, data, size, modal) {
  if (size === "compact") {
    return `<div class="ep-v027-legend compact"><span><i class="actual-combined"></i>${panel._escape(t(panel, "actual"))}</span><span><i class="plan"></i>${panel._escape(t(panel, "plan"))}</span><span><i class="actual-soc"></i>${panel._escape(t(panel, "actualSoc"))}</span><span><i class="forecast-soc"></i>${panel._escape(t(panel, "wantedSoc"))}</span><span><i class="price"></i>${panel._escape(t(panel, "marketPrice"))}</span></div>`;
  }
  ...
  return `<div class="ep-v027-legend">${sourceLegend}<span><i class="plan"></i>${panel._escape(t(panel, "plan"))}</span><span><i class="actual-soc"></i>${panel._escape(t(panel, "actualSoc"))}</span><span><i class="forecast-soc"></i>${panel._escape(t(panel, "wantedSoc"))}</span><span><i class="price"></i>${panel._escape(t(panel, "marketPrice"))}</span></div>`;
}
...
export function ensureStyles(root) {
  ...
  style.textContent = `
    ...
    .ep-v027-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.ep-v027-head-actions{display:flex;align-items:center;gap:8px}.ep-v027-kicker{color:#65e6f9;font-size:10px;letter-spacing:.15em;font-weight:850}.ep-v027-subtitle{color:#819caf;font-size:10px;margin-top:4px}
    ...
    @media(max-width:720px){.ep-v027-battery-plan-card{padding:14px 13px 12px;border-radius:18px}.ep-v027-head{align-items:flex-start}.ep-v027-head-actions{gap:6px}.ep-v027-size-control button{width:27px}.ep-v027-summary,.ep-v027-summary.with-plan{grid-template-columns:1fr}.ep-v027-chip strong{font-size:13px}.ep-v027-chart.size-normal{min-height:220px}.ep-v027-chart.size-large{min-height:260px}}
  `;
