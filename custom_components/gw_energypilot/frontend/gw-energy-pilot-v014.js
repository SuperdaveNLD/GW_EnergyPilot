...
function ensureStyles(root) {
  if (root.querySelector("#ep-v014-style")) return;
  const style = document.createElement("style");
  style.id = "ep-v014-style";
  style.textContent = `
    .ep-v014-costfun {
      margin: 11px 0 4px;
      padding: 10px 11px;
    ...
    .ep-v014-costfun-head strong {
      color: #dff6ff;
      font-size: 10px;
      letter-spacing: .04em;
    }
    .ep-v014-costfun-head span {
      color: #6e8da5;
      font-size: 8px;
    }
    .ep-v014-costfun-actions {
      display: grid;
      grid-template-columns: repeat(3, minmax(0, 1fr));
      gap: 7px;
    }
    .ep-v014-costfun-button {
      min-width: 0;
      min-height: 38px;
      padding: 7px 6px;
      border-radius: 10px;
    ...
    .ep-v014-costfun-button:hover:not(:disabled) {
      border-color: rgba(47, 236, 190, .46);
      color: #efffff;
      background: rgba(10, 68, 81, .62);
    }
    .ep-v014-costfun-button:disabled {
      opacity: .42;
      cursor: not-allowed;
    }
    .ep-v014-costfun-note {
      margin-top: 7px;
      color: #607d94;
      font-size: 8px;
      line-height: 1.4;
    ...
    @media (max-width: 720px) {
      .ep-v014-costfun-actions { grid-template-columns: 1fr; }
    }
  `;
...
function installCostFunctionActions(panel, root) {
  ...
  if (!card || card.querySelector(".ep-v014-costfun")) return;
  ...
  wrap.className = "ep-v014-costfun";
  wrap.innerHTML = `
    <div class="ep-v014-costfun-head">
      <strong>EMHASS cost function</strong>
      <span>config.json · costfun</span>
    </div>
    <div class="ep-v014-costfun-actions"></div>
    <div class="ep-v014-costfun-note">Selecting a strategy writes the complete current EMHASS configuration back with only <strong>costfun</strong> changed, then immediately creates and publishes a fresh optimization. GoodWe control still follows the existing P_batt mapping in v0.14.</div>`;

  const actions = wrap.querySelector(".ep-v014-costfun-actions");
  for (const [key, label, value] of definitions) {
    const entityId = panel._entityId(key);
    const button = document.createElement("button");
    button.type = "button";
    button.className = "ep-v014-costfun-button";
