...
const OPTIMIZE_MARKER = "epV0411StableOptimize";
...
function installStableOptimizeButton(panel, root) {
  ...
  // The inherited v0.10 listener ends every completed optimization with
  // panel._queueRender(). Cloning removes that listener while preserving the
  // existing element presentation. v0.41.1 then owns this single action and
  // updates its state in place, keeping the touched DOM node and HA scroller
  // connected for the complete service call.
  ...
  button.addEventListener("click", async (event) => {
    ...
    if (
      ...
      !entityId ||
      panel.__epV0411OptimizePending ||
      pressedButton.disabled
    ) {
      return;
    }

    panel.__epV0411OptimizePending = true;
    ...
    try {
      ...
      panel.__epV0411OptimizePending = false;
      patchOptimizeUi(panel);
    }
  });
...
function patchOptimizeUi(panel) {
  ...
  const busy = Boolean(panel.__epV0411OptimizePending) || RUNNING_STATES.has(status);
  if (button) {
...
if (PanelClass && !PanelClass.prototype.__epV0411Installed) {
  const previousRender = PanelClass.prototype._render;
  PanelClass.prototype._render = function energyPilotV0411OptimizeStableRender(...args) {
  ...
  PanelClass.prototype.__epV0411Installed = true;
}
