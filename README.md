...
`v1.0.0` remains the stable production release. `v1.1.0-beta.2` is the current
opt-in beta. It retains the v1.1.0-beta.1 feature set and fixes dashboard EP and
EMHASS saves for managed battery profiles. Normal users keep the HACS
prerelease switch off. Testers explicitly enable the HACS prerelease switch for
GW EnergyPilot and can then select a published beta.
...
- `docs/releases/v1.1.0-beta.2.md` — managed-profile settings-save hotfix notes;
...
- The desktop Chromium, iPad WebKit and iPhone WebKit matrix protects the
  complete `1.1.0-beta.2-settings1` frontend graph.
