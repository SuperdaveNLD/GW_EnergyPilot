...
class FrontendV041StableDomTests(unittest.TestCase):
    ...
    def test_v041_bypasses_the_v040_render_settle_layer(self) -> None:
        self.assertIn(
            'import "./gw-energy-pilot-v039.js?v=0.41-stable-dom2"', self.source
        )
    ...
    def test_browser_matrix_uses_one_deterministic_harness(self) -> None:
        ...
        wrapper = (BROWSER / "test_frontend_stability_v041.py").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("frontend_harness.html?entry=v041", wrapper)
        self.assertIn('stability.EXPECTED_ENTRYPOINT = "v041"', wrapper)
        self.assertIn('const ENTRYPOINT = requestedEntry === "v041"', harness)
