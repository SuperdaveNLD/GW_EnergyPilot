...
class FrontendV041StableDomTests(unittest.TestCase):
    ...
    def test_v041_bypasses_the_v040_render_settle_layer(self) -> None:
        self.assertIn(
            'import "./gw-energy-pilot-v039.js?v=1.0.1-beta1"', self.source
        )
    ...
    def test_browser_matrix_uses_one_deterministic_harness(self) -> None:
        ...
        wrapper = (BROWSER / "test_frontend_stability_v101.py").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("frontend_harness.html?entry=v101", wrapper)
        self.assertIn('stability.EXPECTED_ENTRYPOINT = "v101"', wrapper)
        self.assertIn(
            '"v050", "v051", "v100", "v101"].includes(requestedEntry)',
            harness,
        )
        ...
        self.assertIn("test_frontend_stability_v101.py", workflow)
