...
class EVAntiDischargeStrategyTests(unittest.IsolatedAsyncioTestCase):
    ...
    def make_controller(self, *, p_batt: str, p_grid: str, strategy: str):
        ...
        controller.coordinator.last_update_success = True
        controller.coordinator.data.source = "modbus"
        controller.coordinator.data.source_updated_at = datetime.now(timezone.utc)
        controller.coordinator.data.values["total_load_power"] = 11600
    ...
    async def test_hybrid2_ev_planned_charge_with_export_uses_mode2_assistance(self):
        controller, client = self.make_controller(
            p_batt="-2500", p_grid="-4000", strategy="hybrid_2")
        await controller.async_evaluate()
        self.assertEqual(client.calls, [(2, 2500)])
        self.assertEqual(controller.ev_protection_state, "allowing_charge")
        ...
        for strategy in ("battery", "grid", "hybrid", "hybrid_2"):
            for grid in ("-4000", "0", "9574"):
                with self.subTest(strategy=strategy, grid=grid):
                    ...
                    if strategy == "hybrid_2" and grid == "0":
                        self.assertEqual(client.calls, [normal, (5, 600)])
    ...
    async def test_hybrid2_mode2_uses_planned_watts_with_and_without_ev(self):
        for ev in ("0", "11000"):
            for grid in ("2900", "9574"):
                with self.subTest(ev=ev, grid=grid):
                    ...
                    await controller.async_evaluate()
                    self.assertEqual(client.calls, [(2, 1330)])
    ...
    async def test_hybrid2_ev_start_excludes_ev_and_follows_charge_window(self):
        ...
        await asyncio.gather(*controller.hass.tasks)
        self.assertEqual(client.calls, [(1, 0), (5, 600)])
        ...
        await controller.async_evaluate()
        self.assertEqual(client.calls[-1], (2, 4300))
        self.assertEqual(controller.ev_protection_state, "allowing_charge")
    ...
    async def test_hybrid2_ev_blocks_neutral_and_discharge_plans(self):
        for battery in ("-300", "0", "300", "15000"):
            ...
            self.assertEqual(controller.ev_protection_state,
                             "blocking_discharge" if battery == "15000" else "self_consumption_hold")
    ...
    async def test_hybrid2_uses_valid_persistent_grid_plan(self):
        ...
        await controller.async_evaluate()
        self.assertEqual(client.calls, [(2, 8400)])

    async def test_hybrid2_old_mode11_readback_does_not_skip_new_mode2_command(self):
        for ev in ("0", "11000"):
            with self.subTest(ev=ev):
                controller, client = self.make_controller(
                    p_batt="-8400", p_grid="2900", strategy="hybrid_2")
                controller.hass.states.set("sensor.ev_power", ev)
                controller.coordinator.data.mode = 11
                controller.coordinator.data.power = 8400
                preview = controller.mapping_preview()
                self.assertEqual((preview["mode"], preview["power_w"]), (2, 8400))
                self.assertEqual(client.calls, [])
                await controller.async_evaluate()
                self.assertEqual(client.calls, [(2, 8400)])
                self.assertEqual((controller.expected_mode, controller.target_power), (2, 8400))
    ...
    async def test_hybrid2_does_not_remap_manual_commands(self):
        for mode in range(1, 13):
            ...
            await controller.async_evaluate()
            self.assertEqual(client.calls, [(mode, 0 if mode in const.MODES_ZERO_POWER else 4000)])
