...
From v0.14, the EnergyPilot dashboard exposes one button for each objective. Pressing a strategy button performs this sequence:
...
Changing `costfun` changes **what EMHASS optimizes**. It does not by itself change the GoodWe control primitive. In v0.14 Automatic Control still consumes `P_batt` and maps it as follows:
