...
This document is the canonical frontend render/interaction decision for **GW EnergyPilot v0.51 Beta**. v0.51 retains the complete stable behavior, adds one scoped execution-history card and refreshes the entire active module graph with one cache boundary.
...
Every active import uses `0.51-h1`. This is required because v0.51 changes the
nested plan data/view owners and adds the history card; an upgraded browser
must not reuse an older nested module URL.
...
The v0.51 module creates one canonical `.ep-v051-history-card` after the plan
