...
The settings layer is added on top of the v0.16 Beta G20 diagnostics. It does not remove or promote any Beta register semantics.
...
Runtime code continues to read `entry.data` and `entry.options` exactly as before. Existing entity unique IDs remain based on the config-entry ID.
...
The live EMHASS minimum/maximum SOC sliders and cost-function buttons remain on the dashboard. They still write EMHASS `config.json` through the existing EMHASS API path and are intentionally not duplicated in the config-entry settings API.
...
Older EnergyPilot versions identified the Home Assistant device with the mutable `host:slave` pair. That becomes unsafe once the GOODWE page can change the inverter IP or unit ID.

v0.17 migrates the legacy device identifier to the stable GW EnergyPilot config-entry ID **before entities are set up**. The device-registry record therefore remains the same when only the connection address changes.

This preserves the existing device relationship, entity IDs, entity unique IDs and Recorder statistics as far as Home Assistant's registries allow. The migration does not change GoodWe register semantics or control behaviour.
...
The settings model contains all GW EnergyPilot config entries. When more than one is configured, the dashboard shows an entry selector and saves only to the selected entry. Unsaved edits require confirmation before switching entries.
