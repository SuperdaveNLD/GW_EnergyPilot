...
Before promoting **v0.33**, validate on a live installation:
...
14. Run a fresh optimization that changes the plan but leaves the Battery · Plan · Price card already visible. Confirm the existing canonical card refreshes immediately rather than waiting for the normal five-minute cache.
15. Confirm the refresh in step 14 does not create a second `.ep-v027-battery-plan-card`.
16. Confirm a newly reported `P_batt` with the **same numeric value** as the previous row is accepted as fresh after optimization (`last_reported` freshness contract).
17. Confirm the market-price series renders as interval steps and the NOW marker remains aligned with local time.
18. Confirm **Plan charge / Plan discharge** show `—` when no historical/future plan data exists instead of `0.00 kWh`.
19. Compare the GoodWe 35208/35211 headline totals with the Recorder power integral shown beneath them. A difference is allowed because these are separate measurement paths; the native GoodWe counter must remain the headline total.
20. Confirm a Home Assistant restart/integration reload restores the still-valid `gw_energypilot.plan.<entry_id>` mirror before the external plan entities recover, then refreshes it from EMHASS in the bounded startup background path.
21. Confirm missing/expired plan data degrades to an explanatory unavailable state without suppressing actual bars or prices.
22. Confirm Battery/Grid/Hybrid mode mappings, manual EMS commands and support diagnostics remain unaffected by chart reads, plan recovery and window controls.
