...
This document defines the Battery · Plan · Price chart contract used by GW EnergyPilot **v1.1.0** and retained from v1.0.0 Stable.
...
The active v1.1.0 top-level panel URL is versioned and the static integration path disables cache headers. Nested historical modules remain part of the active import chain; do not delete or rename them without tracing that chain.

A live browser session also keeps already-evaluated ES modules in its module map. Changing only the top-level panel URL is therefore not sufficient when a historical nested module itself changes. v1.1.0 loads its presentation wrapper and every inner feature import through `1.1.0-stable1`, including the strategy, settings, scoped plan-refresh and execution-history owners. The older v1.0.0 and v0.33 cache/plan-refresh mechanisms remain historical compatibility context.
