...
This document defines the Battery · Plan · Price chart contract used by GW EnergyPilot **v0.47 Beta**.
...
The active v0.47 top-level panel URL is versioned and the static integration path disables cache headers. Nested historical modules remain part of the active import chain; do not delete or rename them without tracing that chain.

A live browser session also keeps already-evaluated ES modules in its module map. Changing only the top-level panel URL is therefore not sufficient when a historical nested module itself changes. v0.47 refreshes the complete active graph through `0.47-custom-battery1`; the older v0.33 plan-refresh cache-busting mechanism remains historical compatibility context.
